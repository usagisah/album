...
const envValidator = z
  .array(
    z.union([
      z.object({
        common: z.record(z.string(), { invalid_type_error: "config.env.common 必须是一个对象" }).optional(),
        production: z.record(z.string(), { invalid_type_error: "config.env.production 必须是一个对象" }).optional(),
        development: z.record(z.string(), { invalid_type_error: "config.env.development 必须是一个对象" }).optional()
      }),
      z.string()
    ]),
...
const ssrComposeValidator = z
  .object(
    {
      dependencies: z.array(z.string(), { invalid_type_error: "config.ssrCompose.dependencies 必须是一个字符串数组" }).optional()
    },
...
export function checkUserConfig(userConfig: any) {
  const res = z
    .object({
      env: envValidator,
      app: z.union([appValidator, z.array(ssrComposeValidator)], { invalid_type_error: "config.app 必须是一个对象或者数组" }).optional(),
