...
import { SCDName } from "./buildSSRComposeDependencies.js"
...
export async function buildCoordinate(context: AlbumContext) {
  ...
  const { cwd } = inputs
  const { clientOutDir, outDir } = outputs
  ...
  await writeFile(`${outDir}${sep}${SCDName}`, JSON.stringify(_coordinate), "utf-8")
