...
export async function parseMdToReact(mdContent: string, config: ParseMDConfig, albumContext: AlbumContext) {
  ...
  return {
    componentContent: await use({
      extensions: [blockExtension({ highlighter, copyText: copyText, className, categoryQueue, albumContext, demos })],
      renderer: renderer({ highlighter, copyText: copyText, className, categoryQueue, albumContext, demos })
    }).parse(mdContent),
